...
export interface CoverageBrand {
   ...
   *  one size chart, which is what makes this the real unit of research cost. */
...
export function summarizeCoverage(rows: SizingCoverageRow[]): CoverageSummary {
  const brands = new Map<string, CoverageBrand & { formats: Set<string>; categorySet: Set<string> }>();
  ...
  for (const row of rows) {
    ...
    if (!brand) {
      brand = {
        ...
        name: row.brandKey === UNKNOWN_BRAND_KEY ? null : row.brandName,
