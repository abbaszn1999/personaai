...
describe("mapping-options PATCH", () => {
  ...
  // The "Skip to Step 6" button's own gate reads `mapping.sizeChart` from the client store, which
  // only a `GET` used to populate — binding a column here would flip the row to "Attached" (from the
  // optimistic `document` update alone) while the skip button stayed stuck on "Confirm mapping & read
  // catalog" until the next full page load. See the route's own comment on `sizeChartMoved`.
  describe("sizeChart coverage on a changed binding", () => {
    it("recomputes and returns sizeChart when the sizeChartData binding moves", async () => {
      vi.mocked(fetchSampleRawProducts).mockResolvedValue([
        product({ customFields: { "meta.chart": "S/M/L" } }),
        product({ customFields: { "meta.chart": "S/M/L" } }),
        product({ customFields: {} }),
      ]);

      const res = await PATCH(
        patchRequest({ sources: { sizeChartData: { kind: "meta", key: "meta.chart" } } })
      );
      const data = await res.json();

      expect(data.sizeChart).toEqual({ bound: true, withData: 2, sampled: 3 });
    });

    it("omits sizeChart when nothing about the sizeChartData binding changed", async () => {
      vi.mocked(getStoreConnectionByOwner).mockResolvedValue(
        row({ acsFieldMapping: { ...EMPTY_ACS_MAPPING, sources: { brand: { kind: "field", key: "sku" } } } })
      );

      const res = await PATCH(patchRequest({ sources: { brand: { kind: "field", key: "sku" } } }));
      const data = await res.json();

      expect(data.sizeChart).toBeUndefined();
      expect(fetchSampleRawProducts).not.toHaveBeenCalled();
    });

    it("recomputes to bound:false when the binding is cleared", async () => {
      vi.mocked(getStoreConnectionByOwner).mockResolvedValue(
        row({
          acsFieldMapping: { ...EMPTY_ACS_MAPPING, sources: { sizeChartData: { kind: "meta", key: "meta.chart" } } },
        })
      );
      vi.mocked(fetchSampleRawProducts).mockResolvedValue([product()]);

      const res = await PATCH(patchRequest({ sources: {} }));
      const data = await res.json();

      expect(data.sizeChart).toEqual({ bound: false, withData: 0, sampled: 1 });
    });
  });
});
