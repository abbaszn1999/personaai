...
interface NoOpenAiKeyGateProps {
  compact?: boolean;
}

/** Shown instead of the chat panel until the account has a saved OpenAI API key — the
...
export function NoOpenAiKeyGate({ compact = false }: NoOpenAiKeyGateProps) {
  return (
    <div
      className={cn(
    ...
    >
      ...
      <div className="space-y-1 max-w-xs">
        <p className="text-sm font-semibold text-[var(--color-text-primary)]">Add your OpenAI API key to chat</p>
        <p className="text-xs text-[var(--color-text-muted)] leading-relaxed">
          The Style Assistant uses your own OpenAI account for chat, so we need your API key before it can respond.
        </p>
      </div>
      <Link
        href="/settings"
