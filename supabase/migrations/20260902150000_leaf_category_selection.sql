...
-- (`expandCategorySelection`). After this runs, the first save from the leaf UI diffs to an empty
...
expanded as (
  select connection_id, category_id from selected
...
-- New connections start empty, but they will be filled by the leaf UI.
alter table public.store_connections
...
comment on column public.store_connections.category_selection_granularity is
